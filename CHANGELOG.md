...
- **产品新增所需耗材克数**：Product 模型新增 `requiredGrams: Int = 0`（声明处默认值，避免旧数据迁移失败）；新增页和编辑页的成本输入改为耗材克数 Stepper；新增产品页移除颜色和库存输入；消耗弹出层选产品/改数量时自动计算总克数填入输入框（旧产品会尽量从历史消耗或成本估算），超出当前卷剩余量时阻止入库，成本按实际消耗实时分摊换算
